...
I think that we've all struggled with trying to manage versions of a file, as shown in the comic here. For example, a report produced as a Word document evolves over time and in response to reviewers' comments. To maintain versions, what people often do is to rename files, giving them names like like "Final1" or "finalfinal1" or "finalfinal1_revised"...

A more standardized approach might be to append an ISO date to the file name (something like "report_20200319"). This gives the reader at least an understanding of when the last changes occurred, but this approach still provides no description of the latest changes, who made the changes, and why? Wouldn't that info also be useful to know? This approach still does not work for common situations such as when multiple authors are both working independently on the same document on the same date.
...
2. If you want to start a version history of these edits, the first step is create a local repository that holds the files that git creates to manage version histories. You access Git functions from the shell (also referred to as Git BASH, command prompt), which is a command line interface accessible from RStudio and directly from Windows. Once you've navigated to your directory in the shell, the command to initialize a repository is simply **git init**. This creates a hidden folder (.git) in your directory that holds the local repository.
The next step is to add content, referred to as **staging** files. The command to stage files in a file directory is **git add**. What this does is to save the **contents** of each file as blobs (binary large objects) in your local repository. Each blob object is indexed by a unique SHA1 or hash (unique 40 character string). This command then also creates a tree object (you can think of it as an index) that points to the blob objects and provides identifiers and path names for each blob object that you have staged.

3. After staging files that you want to track, the next step is to create a snapshot of the index and blobs and commit these changes to your local repository on your computer. This is called a commit, and you do this with the **git commit** command. In the commit, you are required to create a commit message. This is a description of specifically what was updated and if applicable what bug or problem was addressed. Each tree, commit and blob object has own unique 40 character long SHA-1 representation (We can say that’s the key). Together, those elements are added to a tree data structure which lives inside  the ".git/objects" folder.

4. The rest is just rinse and repeat. You can repeatly stage and commit changes to a local repository. Each commit is chained to the previous commit and points back it to for information that is not changed. 

5. But what if you want to share your work or if you are collaborating with remote partners? This is when you need to share your changes with a remote  repository, such as GitHub (which we will talk more about later). Saving commits from a local repository to a remote repository is a called a **push**. The command **git push** uploads the local repository contents to the remote repository. For the examples today, this would be a repository on GitHub. 
...
???
Ok, so that describes how you stage, commit and push changes, but you might be wondering, how do multiple commits relate to each other?
...
## Adding Version Control to an Existing RStudio Project
